	...
	if [ -d ./PutYourDataBasesHere ];then
		...
		if [ $i -ne 0 ];then            
			...
			if [ $# -eq 1 ]; then
				...
				/opt/spark/bin/spark-submit folderPrimer.py "$checked_dir/data"
			elif [ $# -eq 0 ]; then
				#  Prime the data folder
				/opt/spark/bin/spark-submit folderPrimer.py
			fi
			...
			find PutYourDataBasesHere -type f -exec echo {} \; | cut -f 2- -d "/" | while read -r inputfile;do
				...
				if [ "$(rg $file_SHA_sum -c ./Logs/importedDBS.log)" == "" ];then
					...
					if [ $# -eq 1 ];then
						...
						/opt/spark/bin/spark-submit pysort2.py --export_dir "$1" "$inputfile" 
					else
						#  Call a python script to iterate through the file and sort them
						/opt/spark/bin/spark-submit pysort2.py "$inputfile"
					fi
